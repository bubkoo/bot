...
export async function enforceProtection(
  ...
  const contexts =
    protection && protection.data.required_status_checks
      ? protection.data.required_status_checks.contexts
      : []
  const enforce_admins_current_setting =
    protection && protection.data.enforce_admins.enabled
  ...
  await octokit.repos.updateBranchProtection({
    ...
    required_status_checks: {
      strict: protection
        ? protection.data.required_status_checks.strict
        : false,
      contexts,
    },
    enforce_admins: adminForceChange
      ? !enforce_admins_current_setting
      : enforce_admins_current_setting,
    required_pull_request_reviews: protection?.data
    ...
    required_linear_history:
      protection && protection.data.required_linear_history.enabled,
    allow_force_pushes:
      protection && protection.data.allow_force_pushes.enabled,
    allow_deletions: protection && protection.data.allow_deletions.enabled,
    restrictions:
      protection && protection.data.restrictions
        ? {
            apps: protection.data.restrictions.apps.map(
              ({ slug }: { slug: string }) => slug,
            ),
            users: protection.data.restrictions.users.map(
              ({ login }: { login: string }) => login,
            ),
            teams: protection.data.restrictions.teams.map(
              ({ slug }: { slug: string }) => slug,
            ),
          }
        : null,
